...
public class ChatServer1 {
    ...
    public void onOpen(Session session, @PathParam("username") String username) throws IOException {
        ...
        if (usernameSessionMap.containsKey(username)) {
            session.getBasicRemote().sendText("ENTER NEW USERNAME");
            session.close();
        }
        else {
            ...
            sendMessageToPArticularUser(username, "Please welcome "+username+" to the chat!!!");

            // send to everyone in the chat
            broadcast(username + ": Hello I have joined the chat!");
        }
    }
    ...
    public void onMessage(Session session, String message) throws IOException {
        ...
        if (message.startsWith("@")) {
            ...
            sendMessageToPArticularUser(destUserName, username+"sent this message: "+ actualMessage);
            sendMessageToPArticularUser(username, username+"sent this message: "+ actualMessage);
        }
        else { // Message to whole chat
            broadcast(username + " broadcasted the following: " + message);
        }
    }
    ...
    public void onClose(Session session) throws IOException {
        ...
        broadcast(username + ": I am disconnecting, goodbye.");
    }
